                ...
                .then(returns => {
                  ...
                  console.log('CONFLICTS', events);
                    ...
                    events.forEach(event => {
                      ...
                      if ((newStart > new Date(event.start).getTime() && newEnd < new Date(event.end).getTime()) || (newEnd > new Date(event.start).getTime() && newStart < new Date(event.end).getTime())) {
                        conf = true;
                        console.log('ITS CONFLICT',event);
                      }
                    })
                    if (conf) {
                      ...
                      while( count < 10 ) {
                        ...
                        while (dayCount < 3) {
                          for (var i in aDayTimes) {
                            ...
                            for (var j in events) {
                              if (!((slot.start > new Date(events[j].start).getTime() && slot.end < new Date(events[j].end).getTime()) || (slot.end > new Date(events[j].start).getTime() && slot.start < new Date(events[j].end).getTime()))) {
                      ...
                      console.log('DROPDOWN', dropDown.attachments[0].actions[0].options);
