                ...
                User.findOne({ slackId: message.user }, function(err, foundUser) {
                  ...
                  .then(resp2 => {
                    ...
                    if (response.data.result.parameters.subject){
                      var text = `Meeting with ${inviteeList} to ${response.data.result.parameters.subject} on ${response.data.result.parameters.date}, correct?`
                    } else {
                      var text = `Meeting with ${inviteeList} on ${response.data.result.parameters.date}, correct?`
                    }
